...
namespace STK_ToolBox.ViewModels
{
    ...
    public class CameraViewModel : INotifyPropertyChanged
    {
        ...
        public ICollectionView CamerasView
        {
            ...
            private set { _camerasView = value; OnPropertyChanged("CamerasView"); }
        }
        ...
        public string PlayerStatus
        {
            ...
            set { if (_playerStatus != value) { _playerStatus = value; OnPropertyChanged(); } }
        }

        public CameraItem SelectedCamera
        {
            get { return _selectedCamera; }
            set { if (_selectedCamera != value) { _selectedCamera = value; OnPropertyChanged(); } }
        }
        ...
        public CameraViewModel()
        {
            ...
            OpenSelectedInBrowserCommand = new RelayCommand(() => OpenSelectedInBrowser(),
                () => Cameras.Any(c => c.IsOnline && c.IsSelected));
        }

        private async Task ScanAsync()
        {
            CancelScan(); // 기존 스캔 있으면 취소
                ...
                var tasks = Enumerable.Range(start, total).Select(async i =>
                {
                    ...
                    try
                    {
                        ...
                        if (online)
                        {
                            ...
                            {
                                Cameras.Add(item); // 온라인만 추가 -> 뷰에는 바로 표시
                            }));
        ...
        private void Cameras_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (var obj in e.NewItems)
                {
                    ...
                    if (cam != null) cam.PropertyChanged += CameraItem_PropertyChanged;
                }
            }
            if (e.OldItems != null)
            {
        ...
        private void RefreshViewAndCommands()
        {
            ...
            {
                if (CamerasView != null) CamerasView.Refresh();
                CommandManager.InvalidateRequerySuggested();
            }));
        }
